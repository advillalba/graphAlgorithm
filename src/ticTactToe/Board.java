...
public class Board {
    ...
    public void move(Cell point, CellState player) {
    ...
    public int minimax(int depth, CellState player) {
        ...
        for (int i = 0; i < availableCells.size(); i++) {
            ...
            if (player == CellState.COMPUTER) { //X's turn select the highest from below minimax() call
            ...
            } else if (player == CellState.USER) {//O's turn select the lowest from below minimax() call
                move(point, CellState.USER); 
                scores.add(minimax(depth + 1, CellState.COMPUTER));
            }
            
            board[point.getX()][point.getY()] = CellState.EMPTY; //Reset this point
        }
